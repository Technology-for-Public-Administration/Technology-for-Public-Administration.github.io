...
(function() {
            ...
            shrink: function(t) {
                if (!this.collapsed) {
                    ...
                    var u = this.clone(),
                        ...
                        B = 1;
                    if (v && v.type == 3)
                        if (!x) u.setStartBefore(v);
                        else if (x >= v.getLength()) u.setStartAfter(v);
                    else {
                        u.setStartBefore(v);
                        A = 0;
                    }
                    if (w && w.type == 3)
                        if (!y) u.setEndBefore(w);
                        else if (y >= w.getLength()) u.setEndAfter(w);
                    else {
                        u.setEndAfter(w);
                        B = 0;
                    }
    ...
    j.add('easyUpload', {
        ...
        init: function(l) {
            ...
            m.canUndo = false;
            l.ui.addButton('EasyUpload', {
                label: l.lang.easyUpload.title,
                command: 'easyUpload'
            });
        ...
        function p(r, s) {
            ...
            while (v = t.getNextParagraph()) {
                if (A.useIndentClasses) {
                    ...
                    else v.$.className = e.ltrim(y + ' ' + r.config.indentClasses[x - 1]);
                } else {
            ...
            arrayToList: function(A, B, C, D) {
                ...
                for (;;) {
                    ...
                    if (L.indent == I) {
                        ...
                        J;
                        if (l[L.grandparent.getName()]) J = L.element.clone(false, true);
        ...
        j.add('maximize', {
            init: function(r) {
                ...
                r.addCommand('maximize', {
                    ...
                    editorFocus: false,
                    exec: function() {
                        var A = r.container.getChild(1),
                            B = r.getThemeSpace('contents');
                        if (r.mode == 'wysiwyg') {
                        ...
                        this.toggleState();
                        var J = this.uiItems[0],
                            K = this.state == 2 ? s.maximize : s.minimize,
    ...
    (function() {
        ...
        var q = function() {
            var u = this,
                v = function() {
                    ...
                    if (typeof z == 'object')
                        for (var A in z) y[A] = z[A];
                    if (o) y.id = o;
                    var B = new window.scayt(y),
                        C = r.instances[u.name];
                    if (C) {
                        B.sLang = C.sLang;
                        B.option(C.option());
                        B.paused = C.paused;
        ...
        j.scayt = {
            ...
            loadEngine: function(u) {
                ...
                if (this.engineLoaded === true) return q.apply(u);
        ...
        j.add('scayt', {
            ...
            init: function(u) {
                ...
                var y = u.config.scayt_uiTabs || '1,1,1',
                    z = [];
                y = y.split(',');
                ...
                var E = {};
                E.scaytToggle = {
                    label: u.lang.scayt.enable,
                    command: l,
                    group: D
                };
                if (z[0] == 1) E.scaytOptions = {
                    label: u.lang.scayt.options,
                    group: D,
                    onClick: function() {
                        m = 'options';
                        u.openDialog(l);
                ...
                if (z[2] == 1) E.scaytDict = {
                    label: u.lang.scayt.dictionariesTab,
                    group: D,
                    onClick: function() {
                        m = 'dictionaries';
                        u.openDialog(l);
                ...
                if (u.contextMenu && u.addMenuItems) u.contextMenu.addListener(function() {
                    ...
                    v = {};
                    w = {};
                    var M = u.config.scayt_moreSuggestions || 'on',
                        N = false,
                        O = u.config.scayt_maxSuggestions;
                    typeof O != 'number' && (O = 5);
                    !O && (O = L.length);
        ...
        function v(F) {
            if (F instanceof d.selection) {
                var G = n(F),
                    ...
                    I = o(G);
                for (var J = G.length - 1; J >= 0; J--) v(G[J]);
                if (I) x(I, true);
                else if (H) H.remove();
            } else if (F instanceof h) {
                var K = F.getParent();
        ...
        function y(F) {
            ...
            for (var J = 0; J < G.length; J++) {
                ...
                for (var L = 0; L < G[J].cells.length; L++) {
                    ...
                    K++;
                    while (I[H][K]) K++;
                    var N = isNaN(M.colSpan) ? 1 : M.colSpan,
                        O = isNaN(M.rowSpan) ? 1 : M.rowSpan;
                    for (var P = 0; P < O; P++) {
                        if (!I[H + P]) I[H + P] = [];
            ...
            init: function(F) {
                ...
                F.addCommand('tableDelete', {
                    exec: function(H) {
                        ...
                        L.collapse();
                        I.selectRanges([L]);
                        if (K.getParent().getChildCount() == 1) K.getParent().remove();
                        else K.remove();
                    }
                });
                            ...
                            n.addCommand('toolbarCollapse', {
                                exec: function(H) {
                                    var I = a.document.getById(G),
                                        ...
                                        N = L.$.offsetHeight,
                                        O = !J.isVisible(),
                                        TF = a.document.getById('toolBarFlag').isVisible();
    ...
    (function() {
        ...
        function v(w) {
            ...
            if (J.previous()) {
                ...
                c && u(A);
                var K;
                if (D != 2) K = C.append(new h(D == 1 ? 'p' : 'div'));
                else K = C;
                if (!c) K.appendBogus();
            }
        }
        ...
        j.add('wysiwygarea', {
            ...
            init: function(w) {
                ...
                w.on('editingBlockReady', function() {
                    var B, C, D, E, F, G, H = b.isCustomDomain(),
                        I = function(L) {
                            if (C) C.remove();
                            F = 0;
                        ...
                        K = function(L) {
                            ...
                            var M = L.document,
                                N = M.body,
                                O = M.getElementById('cke_actscrpt');
                            ...
                            Q.on('focus', function() {
                                if (b.gecko) {
                                    ...
                                    if (!V.nextSibling && 'BR' == V.tagName && V.hasAttribute('_moz_editor_bogus_node')) {
                                        ...
                                        if (w.config.enterMode == 2) M.createElement('br', {
                                            attributes: {
                                                _moz_dirty: ''
                                            }
                                        }).replace(X);
                    ...
                    w.addMode('wysiwyg', {
                        load: function(L, M, N) {
                            ...
                            w.mayBeDirty = true;
                            G = true;
                            if (N) this.loadSnapshotData(M);
                            else this.loadData(M);
                        },
                        loadData: function(L) {
                            D = true;
                            var M = w.config,
        ...
        if (b.gecko)(function() {
            var w = document.body;
            if (!w) window.addEventListener('load', arguments.callee, false);
    ...
    (function() {
        ...
        a.dialog.prototype = {
            ...
            hidePage: function(K) {
                ...
                if (!L || M._.pageCount == 1) return;
                else if (K == M._.currentTabId) M.selectPage(m.call(M));
                L.hide();
                M._.pageCount--;
                M.updateStyle();
            },
    ...
    (function() {
        ...
        function o(P) {
            ...
            P.enlarge(1);
            P.trim();
            var X = P.getBoundaryNodes(),
                Y = X.startNode,
                Z = X.endNode.getNextSourceNode(true);
            if (!Z) {
                var aa;
                Z = aa = Q.createText('');
            ...
            if (P.collapsed) {
                ...
                for (var U = 0, V; U < S.elements.length && (V = S.elements[U]); U++) {
                    ...
                    if (this.checkElementRemovable(V)) {
                        var W = P.checkBoundaryOfElement(V, 2),
                            X = !W && P.checkBoundaryOfElement(V, 1);
                        if (X || W) {
                            T = V;
                            T.match = X ? 'start' : 'end';
                        } else {
                ...
                function ac() {
                    var af = new d.elementPath(R.getParent()),
                        ag = new d.elementPath(aa.getParent()),
                        ah = null,
                        ai = null;
                ...
                while (!ad.equals(aa)) {
                    ...
                    if (ad.type == 1 && this.checkElementRemovable(ad)) {
                        if (ad.getName() == this.element) y(this, ad);
                        else A(ad, J(this)[ad.getName()]);
        ...
        function O(P, Q) {
            var R = P.getSelection(),
                S = R.getRanges(),
                T = Q ? this.removeFromRange : this.applyToRange;
